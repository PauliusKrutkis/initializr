...
class WP_REST_MultiplePostType_Controller extends \WP_REST_Controller
{
    ...
    {
        $this->version   = '2';
        $this->namespace = 'query/v' . $this->version;
        $this->rest_base = 'post-query';
    }
    ...
    public function get_items($request)
    {
        ...
        $taxonomies = wp_list_filter(get_object_taxonomies($query_args['post_type'], 'objects'),
    ...
    public function get_collection_params()
    {
        ...
        $taxonomies = wp_list_filter(get_object_taxonomies(get_post_types(array(), 'names'), 'objects'),
            array('show_in_rest' => true));
        foreach ($taxonomies as $taxonomy) {
            $base = ! empty($taxonomy->rest_base) ? $taxonomy->rest_base : $taxonomy->name;
